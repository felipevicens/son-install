...
#Without certificate
docker run -d -t -i --name son-bss --net=sonata --network-alias=son-bss -h $HOSTNAME -p 25001:1337 -p 25002:1338 -v /etc/ssl/private/sonata/:/usr/local/yeoman/SonataBSS/app/certs/ sonatanfv/son-yo-gen-bss:dev sudo grunt serve:integration --gkApiUrl=http://$HOSTNAME/api/v2 --hostname=$HOSTNAME --userManagementEnabled=false --licenseManagementEnabled=false --debug
...
docker run -d --name placementexecutive --net=sonata --network-alias=placementexecutive -e broker_host=amqp://guest:guest@son-broker:5672/%2F sonatanfv/placementexecutive:dev
